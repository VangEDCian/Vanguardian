...
class ImportStudyCrfValidationRulesTemplateCommand:
    actor_user_id: int
...
@dataclass(frozen=True)
class CrfValidationRuleImportIssue:
    sheet_name: str
    row_number: int
    identifier: str
    reason: str
...
@dataclass(frozen=True)
class ImportStudyCrfValidationRulesTemplateResult:
    total_rows: int
    created_count: int
    updated_count: int
    skipped_count: int
    issues: tuple[CrfValidationRuleImportIssue, ...] = ()
    warnings: tuple[str, ...] = ()
